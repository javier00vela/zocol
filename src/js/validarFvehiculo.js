...
$("#registrar").click(function(e){
  ...
  agregarContenido();
  validarVacio();
  if (capacidad != "" && Tpeso != "" && placa != ""  && refrigeracion!= ""  && ciudad != null && licencia != "" && matricula != ""  && soat!="" && prueba != ""){
   matricula = matricula[0].files[0];
   soat = soat[0].files[0];
   licencia = licencia[0].files[0];
   prueba = prueba[0].files[0];
   var datos = new FormData($("#formularioVehiculo")[0]);
   ...
   $.ajax({
                data:  datos,
                url:   'index.php?controller=vehiculo&action=registrarVehiculo',
                type: "post",
                contentType: false,
                processData: false,
